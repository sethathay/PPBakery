				...
				$('#tbl_expense').dataTable( {
					...
					"fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {			
						...
						$("#tbl_expense tr:gt(0)").each(function(){
							totalDiscountDollar += Number($(this).find("td:eq(3)").text().replace(/,/g, ""));
							totalDiscountRiel += Number($(this).find("td:eq(4)").text().replace(/,/g, ""));
							totalRiel += Number($(this).find("td:eq(5)").text().replace(/,/g, ""));
							totalDollar += Number($(this).find("td:eq(6)").text().replace(/,/g, ""));
							totalBalance += Number($(this).find("td:eq(7)").text().replace(/,/g, ""));
						});
						$('#tbl_expense > tbody:last').append('<tr><td colspan="3" style="text-align:right">សរុប:</td><td style="text-align: right;font-weight: bold;">' + (addCommas(totalDiscountRiel)) + '</td><td style="text-align: right;font-weight: bold;">' + (addCommas(getMathRound1000(totalDiscountDollar))) + '</td><td style="text-align: right;font-weight: bold;">' + (addCommas(totalRiel)) + '</td><td style="text-align: right;font-weight: bold;">' + (addCommas(getMathRound1000(totalDollar))) + '</td><td style="text-align: right;font-weight: bold;">' + (addCommas(totalBalance)) + '</td><td></td></tr>');
						
					},
				} );
				/*
				$.ajax({
					url  : '{{ URL::asset("reports/selectReport") }}',
					type : 'post',
					data : {
						dateFrom : dateFrom,
						dateTo	 : dateTo,
					...
					success: function(objResult){
						if(objResult != ""){
							...
							$.each(objResult, function(index, value){
								...
								records += "<td>"+(++index)+"</td>";
								records += "<td>"+value.created_at+"</td>";
								records += "<td>"+value.so_code+"</td>";
								records += "<td>"+value.discount_riel+"</td>";
								records += "<td>"+value.discount_us+"</td>";
								records += "<td>"+value.total_amount_riel+"</td>";
								records += "<td>"+value.total_amount_us+"</td>";
								records += "<td>"+value.balance+"</td>";
								records += "<td></td>";
							...
							$(".table tbody").find("tr").remove();
								var records = "<tr>";
								records += "<td colspan='9'>គ្នានទិន្ន័យនៅក្នុងតារាងទេ</td>";
								records += "</tr>";
								$(".table tbody").append(records);
						}
						waitingDialog.hide();
