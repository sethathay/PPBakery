...
function backup_tables($host,$user,$pass,$name,$tables = '*')
{
	...
	foreach($tables as $table)
	{
		if($table == "inventory_totals" || $table == "inventory_total_details"){
			...
			$result = mysql_query('SELECT * FROM '.$table.' WHERE created_at BETWEEN "'.date("Y-m-d").' 21:00:00" AND "'.date("Y-m-d").'   21:59:59"');
	...
	//save file
	$handle = fopen(dirname(__FILE__).'/db_backup/db-backup-'.date('Y-m-d').'-7.sql','w+');
